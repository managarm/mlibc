...
static void *worker(void *argument) {
	...
	assert(!__cxa_thread_atexit_impl(thread_local_destructor,
			(void *)(intptr_t)1, &dso_handle));
	assert(!__cxa_thread_atexit_impl(thread_local_destructor,
			(void *)(intptr_t)2, &dso_handle));
	return NULL;
}
...
int main(void) {
	...
	assert(!__cxa_thread_atexit_impl(main_thread_destructor, NULL, &dso_handle));
	return 0;
}
