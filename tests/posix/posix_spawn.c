...
static int do_spawn(
	...
	if (wrapper && *wrapper) {
		...
		child_argv = malloc((wrapper_argc + 3) * sizeof(char *));
		assert(child_argv);
...
static void test_file_actions(const char *exec_path) {
	...
	pid_t pid;
	ret = do_spawn(&pid, exec_path, &fa, NULL, "child_file_actions");
	assert(ret == 0);
...
static void test_attributes(const char *exec_path) {
	...
	pid_t pid;
	ret = do_spawn(&pid, exec_path, NULL, &attr, "child_attributes");
	assert(ret == 0);
