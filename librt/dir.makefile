...
# Either build librt after libgcc or include a private version of crtbegin/crtend.
$c_LDFLAGS := -nostdlib

$c_TARGETS := clean-$c install-$c $($c_BINDIR)/librt.so

.PHONY: all-$c install-$c

all-$c: $($c_BINDIR)/librt.so

clean-$c:
	rm -f $($c_BINDIR)/librt.so
...
$($c_BINDIR)/librt.so: $($c_LIBRARY_OBJS) | $($c_BINDIR)
	x86_64-managarm-g++ -shared -o $@ $($c_LDFLAGS) $($c_LIBRARY_OBJS)

install-$c:
	mkdir -p $(SYSROOT_PATH)/usr/lib
	install $($c_BINDIR)/librt.so $(SYSROOT_PATH)/usr/lib
