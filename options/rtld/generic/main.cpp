...
extern "C" void *lazyRelocate(SharedObject *object, unsigned int rel_index) {
	...
	auto p = Scope::resolveGlobalOrLocal(
	    *globalScope, object->localScope, sym.getString(), object->objectRts, 0, ver
	);
	if (!p)
...
static frg::vector<frg::string_view, MemoryAllocator>
parseList(frg::string_view paths, frg::string_view separators) {
...
extern "C" void *interpreterMain(uintptr_t *entry_stack) {
	...
	auto ldso_base = getLdsoBase();
	if (logStartup) {
		mlibc::infoLogger() << "ldso: Own base address is: 0x" << frg::hex_fmt(ldso_base)
		                    << frg::endlog;
	...
	for (size_t i = 0; _DYNAMIC[i].d_tag != DT_NULL; i++) {
		...
		switch (ent->d_tag) {
			...
			case DT_SONAME:
				...
				break;
			case DT_INIT_ARRAY:
				ldso_ctors = reinterpret_cast<ctor_fn *>(ent->d_un.d_ptr + ldso_base);
				break;
			case DT_INIT_ARRAYSZ:
				num_ldso_ctors = ent->d_un.d_val / sizeof(ctor_fn);
				break;
			case DT_HASH:
			case DT_GNU_HASH:
			case DT_STRSZ:
			...
			case DT_FLAGS: {
				...
				mlibc::panicLogger()
				    << "rtld: unexpected DT_FLAGS value of " << frg::hex_fmt(ent->d_un.d_val)
			...
			case DT_FLAGS_1: {
				...
				mlibc::panicLogger()
				    << "rtld: unexpected DT_FLAGS_1 value of " << frg::hex_fmt(ent->d_un.d_val)
				    << " in program interpreter" << frg::endlog;
				break;
			}
			default:
	...
	while (*aux) { // Loop through the environment.
		...
		if (s == size_t(-1))
			mlibc::panicLogger() << "rtld: environment '" << env << "' is missing a '='"
			                     << frg::endlog;
	...
	while (true) {
		...
		if (ldShowAuxv) {
			switch (*aux) {
				case AT_PHDR:
					...
					break;
				case AT_PHENT:
					mlibc::infoLogger() << "AT_PHENT: " << *value << frg::endlog;
					break;
				case AT_PHNUM:
					mlibc::infoLogger() << "AT_PHNUM: " << *value << frg::endlog;
					break;
				case AT_ENTRY:
					mlibc::infoLogger() << "AT_ENTRY: 0x" << frg::hex_fmt{*value} << frg::endlog;
					break;
				case AT_PAGESZ:
					mlibc::infoLogger() << "AT_PAGESZ: " << *value << frg::endlog;
					break;
				case AT_BASE:
					mlibc::infoLogger() << "AT_BASE: 0x" << frg::hex_fmt{*value} << frg::endlog;
					break;
				case AT_FLAGS:
					mlibc::infoLogger() << "AT_FLAGS: 0x" << frg::hex_fmt{*value} << frg::endlog;
					break;
				case AT_NOTELF:
					mlibc::infoLogger() << "AT_NOTELF: " << frg::hex_fmt{*value} << frg::endlog;
					break;
				case AT_UID:
					mlibc::infoLogger() << "AT_UID: " << *value << frg::endlog;
					break;
				case AT_EUID:
					mlibc::infoLogger() << "AT_EUID: " << *value << frg::endlog;
					break;
				case AT_GID:
					mlibc::infoLogger() << "AT_GID: " << *value << frg::endlog;
					break;
				case AT_EGID:
					mlibc::infoLogger() << "AT_EGID: " << *value << frg::endlog;
					break;
#ifdef AT_PLATFORM
				case AT_PLATFORM:
					mlibc::infoLogger()
					    << "AT_PLATFORM: " << reinterpret_cast<const char *>(*value) << frg::endlog;
					break;
#endif
#ifdef AT_HWCAP
				case AT_HWCAP:
					mlibc::infoLogger() << "AT_HWCAP: " << frg::hex_fmt{*value} << frg::endlog;
					break;
#endif
#ifdef AT_CLKTCK
				case AT_CLKTCK:
					mlibc::infoLogger() << "AT_CLKTCK: " << *value << frg::endlog;
					break;
#endif
#ifdef AT_FPUCW
				case AT_FPUCW:
					mlibc::infoLogger() << "AT_FPUCW: " << frg::hex_fmt{*value} << frg::endlog;
					break;
#endif
#ifdef AT_SECURE
				case AT_SECURE:
					mlibc::infoLogger() << "AT_SECURE: " << *value << frg::endlog;
					break;
#endif
#ifdef AT_RANDOM
				case AT_RANDOM:
				...
				case AT_EXECFN:
					mlibc::infoLogger()
					    << "AT_EXECFN: " << reinterpret_cast<const char *>(*value) << frg::endlog;
					break;
#endif
#ifdef AT_SYSINFO_EHDR
				case AT_SYSINFO_EHDR:
					mlibc::infoLogger()
					    << "AT_SYSINFO_EHDR: 0x" << frg::hex_fmt{*value} << frg::endlog;
					break;
#endif
			}
		}
		...
		switch (*aux) {
			...
			case AT_RANDOM:
				...
				break;
			case AT_SECURE:
				rtldConfig.secureRequired = reinterpret_cast<uintptr_t>(*value);
				break;
		}
...
// using the host toolchain.
	...
	for (size_t i = 0; i < num_ldso_ctors; i++) {
		if (logStartup)
			mlibc::infoLogger() << "ldso: Running own constructor at "
			                    << reinterpret_cast<void *>(ldso_ctors[i]) << frg::endlog;
		ldso_ctors[i]();
	}
#else
	if (num_ldso_ctors > 0) {
		mlibc::panicLogger()
		    << "ldso: Found unexpected own global constructor(s), init_array starts at: "
		    << ldso_ctors << frg::endlog;
	}
#endif
	...
	if (logStartup)
		mlibc::infoLogger() << "ldso: Executable PHDRs are at " << phdr_pointer << frg::endlog;
	...
	if (logEntryExit)
		mlibc::infoLogger() << "Leaving ld.so, jump to " << (void *)executableSO->entry
		                    << frg::endlog;
...
void *__dlapi_open(const char *file, int flags, void *returnAddress) {
	if (logDlCalls)
		mlibc::infoLogger() << "rtld: __dlapi_open(" << (file ? file : "nullptr") << ")"
		                    << frg::endlog;
	...
	if (flags & RTLD_NOLOAD) {
		...
		if (object && object->globalRts == 0 && (flags & RTLD_GLOBAL)) {
			// The object was opened with RTLD_LOCAL, but we are called with RTLD_NOLOAD |
			// RTLD_GLOBAL. According to the man page, we should promote to the global scope here.
		...
		if (frg::string_view{file}.find_first('/') == size_t(-1)) {
			...
			objectResult =
			    initialRepository->requestObjectWithName(file, origin, newScope, !isGlobal, rts);
		} else {
...
void *__dlapi_resolve(void *handle, const char *string, void *returnAddress, const char *version) {
	...
	if (handle == RTLD_DEFAULT) {
		...
		struct Token {};
		frg::hash_map<SharedObject *, Token, frg::hash<SharedObject *>, MemoryAllocator> visited{
		    frg::hash<SharedObject *>{}, getAllocator()
		};
...
int __dlapi_reverse(const void *ptr, __dlapi_symbol *info) {
	...
	for (size_t i = 0; i < initialRepository->loadedObjects.size(); i++) {
		...
		for (size_t i = 0; i < num_symbols; i++) {
			ObjectSymbol cand{
			    object,
			    (elf_sym *)(object->baseAddress + object->symbolTableOffset + i * sizeof(elf_sym))
			};
			if (eligible(cand) && cand.virtualAddress() == reinterpret_cast<uintptr_t>(ptr)) {
				if (logDlCalls)
					mlibc::infoLogger() << "rtld: Found symbol " << cand.getString()
...
_dl_find_object(void *address, dl_find_object *result) {
