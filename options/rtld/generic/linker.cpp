...
SharedObject *ObjectRepository::injectObjectFromDts(
	...
	auto object = frg::construct<SharedObject>(
	    getAllocator(), name.data(), std::move(path), false, globalScope.get(), rts
	);
...
SharedObject *ObjectRepository::injectObjectFromPhdrs(
	...
	auto object = frg::construct<SharedObject>(
	    getAllocator(), name.data(), std::move(path), true, globalScope.get(), rts
	);
...
SharedObject *ObjectRepository::injectStaticObject(
	...
	object->initArraySize =
	    static_cast<size_t>((uintptr_t)__init_array_end - (uintptr_t)__init_array_start);
	object->finiArray = reinterpret_cast<InitFuncPtr *>(__fini_array_start);
	object->finiArraySize =
	    static_cast<size_t>((uintptr_t)__fini_array_end - (uintptr_t)__fini_array_start);
...
frg::expected<LinkerError, SharedObject *> ObjectRepository::requestObjectWithName(
	...
	auto object = frg::construct<SharedObject>(
	    getAllocator(), name.data(), std::move(chosenPath), false, localScope, rts
	);
...
frg::expected<LinkerError, SharedObject *> ObjectRepository::requestObjectAtPath(
...
void ObjectRepository::_fetchFromPhdrs(
    SharedObject *object,
    void *phdr_pointer,
    size_t phdr_entry_size,
    size_t phdr_count,
...
frg::expected<LinkerError, void> ObjectRepository::_fetchFromFile(SharedObject *object, int fd) {
	...
	for (int i = 0; i < ehdr.e_phnum; i++) {
		...
		if (phdr->p_type == PT_LOAD) {
			...
			memset(
			    reinterpret_cast<void *>(map_address + misalign + phdr->p_filesz),
			    0,
...
void ObjectRepository::_parseVerdef(SharedObject *object) {
	...
	uintptr_t address = object->baseAddress + object->versionDefinitionTableOffset;
...
void ObjectRepository::_parseVerneed(SharedObject *object) {
	...
	uintptr_t address = object->baseAddress + object->versionRequirementTableOffset;
	...
	for (size_t i = 0; i < object->versionRequirementCount; i++) {
		...
		for (size_t j = 0; j < need.vn_cnt; j++) {
			...
			if (!ver)
				mlibc::panicLogger()
				    << "mlibc: Object " << target->name << " does not define version \"" << name
				    << "\" needed by object " << object->name << frg::endlog;

			bool isDefault = !(aux.vna_other & 0x8000);
			// Bit 15 indicates whether the static linker should ignore this version.
			object->knownVersions.insert(
			    aux.vna_other & 0x7FFF, isDefault ? ver->makeDefault() : *ver
			);
...
void
ObjectRepository::_discoverDependencies(SharedObject *object, Scope *localScope, uint64_t rts) {
...
: name(name, getAllocator()),
  ...
  symbolTableOffset(0),
  stringTableOffset(0),
  knownVersions({}, getAllocator()),
  definedVersions(getAllocator()),
...
SharedObject::SharedObject(
    const char *name, const char *path, bool is_main_object, Scope *localScope, uint64_t object_rts
)
: SharedObject(
      name,
...
frg::tuple<ObjectSymbol, SymbolVersion> SharedObject::getSymbolByIndex(size_t index) {
	...
	ObjectSymbol sym{
	    this, reinterpret_cast<elf_sym *>(baseAddress + symbolTableOffset + index * sizeof(elf_sym))
	};
	...
	if (versionTableOffset) {
		...
		memcpy(
		    ...
		    reinterpret_cast<void *>(
		        baseAddress + versionTableOffset + index * sizeof(elf_version)
		    ),
		    sizeof(elf_version)
		);
		...
		if (verIdx != 0 && verIdx != 1) {
			...
			if (maybeVer == knownVersions.end())
				mlibc::panicLogger() << "mlibc: Symbol " << sym.getString() << " of object " << name
...
// TODO: R_IRELATIVE also exists on other architectures but will likely need a different
// implementation.
#if defined(__x86_64__) || defined(__i386__)
		case R_IRELATIVE: {
...
Tcb *allocateTcb() {
	...
	tcb_ptr->localKeys =
	    frg::construct<frg::array<Tcb::LocalKey, PTHREAD_KEYS_MAX>>(getAllocator());
...
frg::optional<ObjectSymbol> resolveInObject(
	...
	if (object->hashStyle == HashStyle::systemV) {
		...
		auto hash_table =
		    reinterpret_cast<const GnuTable *>(object->baseAddress + object->hashTableOffset);
...
frg::optional<ObjectSymbol> Scope::_resolveNext(
	...
	if (i == _objects.size()) {
		mlibc::infoLogger() << "rtld: object passed to Scope::resolveAfter was not found"
		                    << frg::endlog;
		return frg::optional<ObjectSymbol>();
	}
...
frg::optional<ObjectSymbol> Scope::resolveGlobalOrLocal(
    Scope &globalScope,
    Scope *localScope,
...
frg::optional<ObjectSymbol> Scope::resolveGlobalOrLocalNext(
    Scope &globalScope,
    Scope *localScope,
    frg::string_view string,
    SharedObject *origin,
...
frg::optional<ObjectSymbol> Scope::resolveSymbol(
    frg::string_view string,
    uint64_t skipRts,
    ResolveFlags flags,
    frg::optional<SymbolVersion> version
		...
		for (size_t offset = 0; offset < *relr_length; offset += sizeof(elf_relr)) {
			...
			if (!(entry & 1)) {
				...
				*addr++ += object->baseAddress;
				...
				// Odd entry indicates entry is a bitmap of the subsequent locations to be
				// relocated.

				// The first bit of an entry is always a marker about whether the entry is an
				// address or a bitmap, discard it.
				entry >>= 1;
...
// TODO: TLSDESC relocations aren't aarch64/x86_64 specific
			...
			default:
				mlibc::panicLogger()
				    << "unimplemented lazy relocation type " << type << frg::endlog;
				break;
		}
	}
}
