...
constexpr auto staticSysconfValues = std::to_array<std::pair<int, long>>({
	// {_SC_AIO_LISTIO_MAX, AIO_LISTIO_MAX},
	// {_SC_AIO_MAX, AIO_MAX},
	// {_SC_AIO_PRIO_DELTA_MAX, AIO_PRIO_DELTA_MAX},
	{_SC_ARG_MAX, 2097152}, // On linux, it is defined to 2097152 in most cases, so define it to be 2097152
...
long sysconf(int number) {
	...
	auto it = std::ranges::find(staticSysconfValues, number, &std::pair<int, long>::first);
	if (it != staticSysconfValues.end())
		return it->second;
