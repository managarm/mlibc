...
uintptr_t getLdsoBase() {
  ...
  // TODO: This isn't guaranteed on AArch64, so this might fail with some
  // linkers.
...
// Note that this code is fragile in the sense that it must not contain
// relocations itself.
...
extern "C" void *lazyRelocate(SharedObject *object, unsigned int rel_index) {
  ...
  auto reloc =
      (Elf64_Rela *)(object->baseAddress + object->lazyRelocTableOffset +
...
extern "C" [[gnu::alias("dl_debug_state"), gnu::visibility("default")]] void
_dl_debug_state() noexcept;

// This symbol can be used by GDB to find the global interface structure
[[gnu::visibility("default")]] DebugInterface *_dl_debug_addr =
...
/// Returns weather the given environment variable (`env`) is present and
/// non-empty.
bool nonzero_env(char **rtdl_envp, frg::string_view env) {
  if (!rtdl_envp)
    return false;

  for (size_t i = 0; rtdl_envp[i]; i++) {
    frg::string_view current_entry{rtdl_envp[i]};
    size_t namelen = current_entry.find_first('=');

    if (namelen == size_t(-1)) {
      continue;
    }

    // NOTE: ensure that the envionment variable non-empty
    if (current_entry.sub_string(0, namelen) == env &&
        current_entry.size() >= env.size() + 2) {
      return true;
    }
  }
...
/// Initializes all of the logger configuration variables.
void init_log(char **rtdl_envp) {
  log_entry_exit = nonzero_env(rtdl_envp, "MLIBC_RTDL_LOG_ENTRY_EXIT");
  log_startup = nonzero_env(rtdl_envp, "MLIBC_RTDL_LOG_STARTUP");
}
} // namespace
...
extern "C" void *interpreterMain(uintptr_t *entry_stack) {
  ...
  if (logStartup) {
    ...
    mlibc::infoLogger() << "ldso: Own dynamic section is at: " << _DYNAMIC
                        << frg::endlog;
  }

// on aarch64 these lines corrupt unrelated GOT entries (entries for ld.so
// functions)
#ifdef __x86_64__
  ...
  // Here, we make sure that the dynamic linker does not need relocations
  // itself.
  ...
  for (size_t i = 0; _DYNAMIC[i].d_tag != DT_NULL; i++) {
    ...
    switch (ent->d_tag) {
    case DT_STRTAB:
      strtab_offset = ent->d_un.d_ptr;
      break;
    case DT_SONAME:
      soname_str = ent->d_un.d_val;
      break;
    case DT_HASH:
    case DT_GNU_HASH:
    case DT_STRSZ:
  ...
  init_log(reinterpret_cast<char **>(aux));
  ...
  while (true) {
    ...
    switch (*aux) {
    case AT_PHDR:
      phdr_pointer = reinterpret_cast<void *>(*value);
      break;
    case AT_PHENT:
      phdr_entry_size = *value;
      break;
    case AT_PHNUM:
      phdr_count = *value;
      break;
    case AT_ENTRY:
      entry_pointer = reinterpret_cast<void *>(*value);
      break;
    case AT_EXECFN:
      execfn = reinterpret_cast<const char *>(*value);
      break;
    case AT_RANDOM:
      stack_entropy = reinterpret_cast<void *>(*value);
      break;
    case AT_SECURE:
      secureRequired = reinterpret_cast<uintptr_t>(*value);
      break;
    }
...
#ifndef MLIBC_STATIC_BUILD
  auto ldso_soname =
      reinterpret_cast<const char *>(ldso_base + strtab_offset + soname_str);
  auto ldso = initialRepository->injectObjectFromDts(
      ldso_soname, frg::string<MemoryAllocator>{getAllocator()}, ldso_base,
      _DYNAMIC, 1);

  // TODO: support non-zero base addresses?
  executableSO = initialRepository->injectObjectFromPhdrs(
      execfn, frg::string<MemoryAllocator>{execfn, getAllocator()},
...
extern "C" [[gnu::visibility("default")]] const char *__dlapi_error() {
...
__dlapi_get_tls(struct __abi_tls_entry *entry) {
  return reinterpret_cast<char *>(accessDtv(entry->object)) + entry->offset;
}

#ifdef __MLIBC_POSIX_OPTION

extern "C" [[gnu::visibility("default")]] void *__dlapi_open(const char *file,
                                                             int local) {
  // TODO: Thread-safety!
  auto rts = rtsCounter++;
  ...
  if (local)
    mlibc::infoLogger() << "\e[31mrtdl: RTLD_LOCAL " << file
                        << " is not supported properly\e[39m" << frg::endlog;
  ...
  if (!object->objectScope) {
    ...
    // Loop over indices (not iterators) here: We are adding elements in the
    // loop!
...
extern "C" [[gnu::visibility("default")]] void *
__dlapi_resolve(void *handle, const char *string) {
  if (logDlCalls)
    mlibc::infoLogger() << "rtdl: __dlapi_resolve(" << handle << ", " << string
                        << ")" << frg::endlog;
  ...
  if (handle == RTLD_DEFAULT) {
    ...
    // POSIX does not unambiguously state how dlsym() is supposed to work; it
    // just states that "The symbol resolution algorithm used shall be
    // dependency order as described in dlopen()".
    //
    // Linux libc's lookup the symbol in the given DSO and all of its
    // dependencies in breadth-first order. That is also what we implement here.
    //
    ...
    // and (ii) the local scope of the DSO into account (which can contain more
    // objects than just the dependencies of the DSO, if the DSO was loaded as a
    // dependency of a dlopen()ed DSO).

    frg::vector<SharedObject *, MemoryAllocator> queue{getAllocator()};

    struct Token {};
    frg::hash_map<SharedObject *, Token, frg::hash<SharedObject *>,
                  MemoryAllocator>
  ...
  if (!target) {
    if (logDlCalls)
      mlibc::infoLogger() << "rtdl: could not resolve \"" << string << "\""
                          << frg::endlog;
...
__dlapi_reverse(const void *ptr, __dlapi_symbol *info) {
  if (logDlCalls)
    mlibc::infoLogger() << "rtdl: __dlapi_reverse(" << ptr << ")"
                        << frg::endlog;
  ...
  for (size_t i = 0; i < globalScope->_objects.size(); i++) {
    ...
    for (size_t i = 0; i < num_symbols; i++) {
      ObjectSymbol cand{object, (Elf64_Sym *)(object->baseAddress +
                                              object->symbolTableOffset +
                                              i * sizeof(Elf64_Sym))};
      if (eligible(cand) &&
          cand.virtualAddress() == reinterpret_cast<uintptr_t>(ptr)) {
        if (logDlCalls)
          mlibc::infoLogger() << "rtdl: Found symbol " << cand.getString()
                              << " in object " << object->path << frg::endlog;
  ...
  for (size_t i = 0; i < globalScope->_objects.size(); i++) {
    ...
    for (size_t j = 0; j < object->phdrCount; j++) {
      auto phdr = (Elf64_Phdr *)((uintptr_t)object->phdrPointer +
      ...
      if (reinterpret_cast<uintptr_t>(ptr) >= start &&
          reinterpret_cast<uintptr_t>(ptr) < end) {
        mlibc::infoLogger()
            << "rtdl: Found DSO " << object->path << frg::endlog;
        info->file = object->path.data();
        info->base = reinterpret_cast<void *>(object->baseAddress);
        info->symbol = nullptr;
        info->address = 0;
  ...
  if (logDlCalls)
    mlibc::infoLogger() << "rtdl: Could not find symbol in __dlapi_reverse()"
                        << frg::endlog;
...
extern "C" [[gnu::visibility("default")]] int
__dlapi_iterate_phdr(int (*callback)(struct dl_phdr_info *, size_t, void *),
                     void *data) {
  int last_return = 0;
...
extern "C" [[gnu::visibility("default")]] void
__dlapi_enter(uintptr_t *entry_stack) {
