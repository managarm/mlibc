...
Part of configuring an [OS Specific Toolchain](https://osdev.wiki/OS_Specific_Toolchain) is [specifying the location of `(S)crt1.o`](https://osdev.wiki/OS_Specific_Toolchain#Start_Files_Directory) so that it is linked automatically.
...
Make sure to take a look at the [ABI specification](https://osdev.wiki/System_V_ABI#See_Also) for your architecture for details like proper stack layout. People often forget to implement things like auxiliary vectors which are required by the spec.
