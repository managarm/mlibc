...
namespace mlibc
{
    ...
    int sys_pwrite(int fd, const void* buf, size_t n, off_t off,
    ...
    int sys_readlink(const char* path, void* buffer, size_t maxSize,
                     ssize_t* length)
    {
    ...
    {
        auto ret
            = Syscall(SYS_RENAMEAT, olddirfd, old_path, newdirfd, new_path);
        if (auto e = syscall_error(ret); e) return e;
    ...
    {
        auto ret = Syscall(SYS_LINKAT, olddirfd, old_path, newdirfd, new_path,
                           flags);
        if (auto e = syscall_error(ret); e) return e;
    ...
    int sys_readlinkat(int dirfd, const char* path, char* buf, size_t bufsz,
                       ssize_t* length)
    {
    ...
    STUB_RET(int sys_pselect([[maybe_unused]] int                    nfds,
                             [[maybe_unused]] fd_set*                readfds,
                             [[maybe_unused]] fd_set*                writefds,
                             [[maybe_unused]] fd_set*                exceptfds,
                             [[maybe_unused]] const struct timespec* timeout,
                             [[maybe_unused]] const sigset_t*        sigmask,
                             [[maybe_unused]] int* num_events));
