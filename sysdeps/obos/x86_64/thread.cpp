...
int sys_prepare_stack(void **stack, void *entry, void *user_arg, void* tcb, size_t *stack_size, size_t *guard_size, void **stack_base)
{
    ...
    if (!*stack_size) *stack_size = 1024*1024*4;
    if (!(*stack))
    {
        ...
        *stack_base = (void*)syscall5(Sys_VirtualMemoryAlloc, HANDLE_CURRENT, NULL, *stack_size+(*guard_size == 0x1000 ? 0 : *guard_size), &map_args, &status);
        ...
        if (map_args.flags & (1<<8))
        {
            ...
            map_args.flags = 0;
            syscall5(Sys_VirtualMemoryAlloc, HANDLE_CURRENT, (void*)((uintptr_t)*stack_base + *guard_size), *stack_size, &map_args, &status);
            if (obos_is_error(status))
                return EAGAIN;
        }
    }
