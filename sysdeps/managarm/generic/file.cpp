...
int sys_timerfd_settime(
	...
	if (oldvalue) {
		oldvalue->it_value.tv_sec = req.value_sec();
		oldvalue->it_value.tv_nsec = req.value_nsec();
		oldvalue->it_interval.tv_sec = req.interval_sec();
		oldvalue->it_interval.tv_nsec = req.interval_nsec();
	}
