...
int Sysdeps<Ftruncate>::operator()(int fd, size_t size) { return menix_syscall(SYSCALL_FTRUNCATE, fd, size).error; }
...
int Sysdeps<Listen>::operator()(int fd, int backlog) { return menix_syscall(SYSCALL_LISTEN, fd, backlog).error; }
...
int Sysdeps<Shutdown>::operator()(int sockfd, int how) { return menix_syscall(SYSCALL_SHUTDOWN, sockfd, how).error; }
...
gid_t Sysdeps<GetGid>::operator()() { return menix_syscall(SYSCALL_GETGID).value; }

gid_t Sysdeps<GetEgid>::operator()() { return menix_syscall(SYSCALL_GETEGID).value; }

uid_t Sysdeps<GetUid>::operator()() { return menix_syscall(SYSCALL_GETUID).value; }

uid_t Sysdeps<GetEuid>::operator()() { return menix_syscall(SYSCALL_GETEUID).value; }

pid_t Sysdeps<GetPid>::operator()() { return menix_syscall(SYSCALL_GETPID).value; }

pid_t Sysdeps<GetTid>::operator()() { return menix_syscall(SYSCALL_GETTID).value; }

pid_t Sysdeps<GetPpid>::operator()() { return menix_syscall(SYSCALL_GETPPID).value; }
...
int Sysdeps<SetPgid>::operator()(pid_t pid, pid_t pgid) { return menix_syscall(SYSCALL_SETPGID, pid, pgid).error; }
int Sysdeps<SetUid>::operator()(uid_t uid) { return menix_syscall(SYSCALL_SETUID, uid).error; }
int Sysdeps<SetEuid>::operator()(uid_t euid) { return menix_syscall(SYSCALL_SETEUID, euid).error; }
int Sysdeps<SetGid>::operator()(gid_t gid) { return menix_syscall(SYSCALL_SETGID, gid).error; }
int Sysdeps<SetEgid>::operator()(gid_t egid) { return menix_syscall(SYSCALL_SETEGID, egid).error; }
...
void Sysdeps<Yield>::operator()() { menix_syscall(SYSCALL_YIELD); }
...
int Sysdeps<Chdir>::operator()(const char *path) { return menix_syscall(SYSCALL_CHDIR, (size_t)path).error; }

int Sysdeps<Fchdir>::operator()(int fd) { return menix_syscall(SYSCALL_FCHDIR, fd).error; };

int Sysdeps<Chroot>::operator()(const char *path) { return menix_syscall(SYSCALL_CHROOT, (size_t)path).error; }
...
void Sysdeps<Sync>::operator()() { menix_syscall(SYSCALL_SYNC); }

int Sysdeps<Fsync>::operator()(int fd) { return menix_syscall(SYSCALL_FSYNC, fd).error; }

int Sysdeps<Fdatasync>::operator()(int fd) { return menix_syscall(SYSCALL_FDATASYNC, fd).error; }

int Sysdeps<Chmod>::operator()(const char *pathname, mode_t mode) {
	return sysdep<Fchmodat>(AT_FDCWD, pathname, mode, 0);
}

int Sysdeps<Fchmod>::operator()(int fd, mode_t mode) { return menix_syscall(SYSCALL_FCHMOD, fd, mode).error; }
...
int Sysdeps<Pipe>::operator()(int *fds, int flags) { return menix_syscall(SYSCALL_PIPE, (size_t)fds, flags).error; }
...
int Sysdeps<Sigaction>::operator()(int sig, const struct sigaction *__restrict act, struct sigaction *__restrict oact) {
...
int Sysdeps<Kill>::operator()(pid_t pid, int signal) { return menix_syscall(SYSCALL_KILL, pid, signal).error; }
...
int Sysdeps<Sigpending>::operator()(sigset_t *set) { return menix_syscall(SYSCALL_SIGPENDING, (size_t)set).error; }
...
int Sysdeps<TimerDelete>::operator()(timer_t t) { return menix_syscall(SYSCALL_TIMER_DELETE, (size_t)t).error; }
