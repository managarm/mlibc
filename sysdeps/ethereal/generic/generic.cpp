...
int Sysdeps<VmMap>::operator()(
    void *addr, size_t size, int prot, int flags, int fd, off_t offset, void **window
) {
	struct {
		void *addr;
		size_t len;
...
int Sysdeps<AnonAllocate>::operator()(size_t size, void **pointer) {
	return sysdep<VmMap>(
	    nullptr, size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0, pointer
	);
}
...
int Sysdeps<TcbSet>::operator()(void *pointer) { return -SYSCALL1(SYS_SETTLS, (uintptr_t)pointer); }
...
int Sysdeps<Chdir>::operator()(const char *path) { return -SYSCALL1(SYS_CHDIR, path); }

int Sysdeps<Fchdir>::operator()(int fd) { return -SYSCALL1(SYS_FCHDIR, fd); }
...
int
Sysdeps<Fchownat>::operator()(int dirfd, const char *pathname, uid_t uid, gid_t gid, int flags) {
...
int Sysdeps<GetEntropy>::operator()(void *buffer, size_t length) {
	...
	if (err) {
		mlibc::infoLogger() << "mlibc: reading from /device/random failed: " << strerror(errno)
		                    << frg::endlog;
		return err;
	}
...
int Sysdeps<Clone>::operator()(void *tcb, pid_t *pid_out, void *stack) {
	long ret = SYSCALL4(
	    SYS_CREATE_THREAD, (uintptr_t)stack, (uintptr_t)tcb, (void *)__mlibc_thread_entry, nullptr
	);
	if (ret < 0)
		return -ret;
